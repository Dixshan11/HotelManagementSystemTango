                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex justify-content-between">
                                    <div>
                                        <h3> <span style="color:red">Update </span> <span style="color:blue">User </span></h3>
                                    </div>
                                    <div>
                                        <a href="{{route('users.index')}}" class="btn btn-sm btn-info float-end">Back</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                        ...
                                        <div class="col-12 col-md-4" id="role">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control" onchange="roleGetPermission()" name="role">
                                                    ...
                                                    <option value="{{$obj->id}}" {{old('role',$user->roles->first()->id) == $obj->id ? 'selected' : ''}}>
                                                        {{$obj->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <roleset>
                                                        ...
                                                        <div>
                                                            <label class="checkboxLabel">
                                                                <input type="checkbox" id="permission-{{$permission->id}}" class="permission-check permission-selected-{{str_replace('','-',$key)}}"
                                                                 name="permissions[]" value="{{$permission->id}}" {{in_array($permission->id, $user->permissions->pluck('id')->toArray()) ? 'checked' : ''}}>
                                                                {{$permission->name}}
                                                            </label>
                                                        </div>
                                                        @empty
...
<script>
    ...
    function roleGetPermission() {
        ...
        sendHttpRequest("/role-permissions", "POST", {
            role_id: role
        }, getData, isToShowLoader = true);
    }
